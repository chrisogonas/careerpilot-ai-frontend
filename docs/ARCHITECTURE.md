...
5. **Inactivity Auto-Logout** (30 minutes)
   - Custom `useInactivityTimeout` hook in `lib/hooks/useInactivityTimeout.ts`
   - Monitors 6 DOM events: `mousemove`, `mousedown`, `keydown`, `scroll`, `touchstart`, `pointerdown`
   - After 28 minutes of inactivity, an amber warning toast appears (bottom-right)
   - After 30 minutes of inactivity, user is automatically logged out
   - Tokens are cleared from localStorage and user is redirected to `/auth/login?reason=inactivity`
   - Login page displays an informational banner when redirected due to inactivity
   - Any user interaction during the warning period resets the timer and dismisses the warning
   - Hook is only active when user is authenticated (`enabled: !!user`)
